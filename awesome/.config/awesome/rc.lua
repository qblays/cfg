...
--awful.util.tagnames = {  " ", " ", " ", " ", " ", " ", " ", " ", " ", " "  }
--awful.util.tagnames = { " MAIN ", " SEC ", " SSH ", " 4 ", " 5 ", " 6 ", " 7  ", " DOC ", " REM ", " 9 " }
...
--tags = {
  --settings = {
    --{ names  = { "www", "editor", "mail", "im" },
      --layout = { awful.layout.suit.floating, awful.layout.suit.floating, awful.layout.suit.floating, awful.layout.suit.floating }
    --},
    --{ names  = { "rss",  "6", "7",  "media" },
      --layout = { awful.layout.suit.floating, awful.layout.suit.floating, awful.layout.suit.floating, awful.layout.suit.floating }
--}}}
--local tags = sharedtags({
    --{ name = "main", layout = awful.layout.layouts[0] },
    --{ name = "www", layout = awful.layout.layouts[1] },
    --{ name = "game", layout = awful.layout.layouts[0] },
    --{ name = "misc", layout = awful.layout.layouts[1] },
    --{ name = "chat", screen = 2, layout = awful.layout.layouts[1] },
    --{ layout = awful.layout.layouts[1] },
    --{ screen = 2, layout = awful.layout.layouts[0] }
--})
...
awful.screen.connect_for_each_screen(function(s)
	-- charitable Show an unselected tag when a screen is connected
    for i = 1, #tags do
...
globalkeys = my_table.join(
    ...
    awful.key({ winkey, altkey }, "a", function () awful.util.spawn( terminal.." -e ncpamixer" ) end,
        {description = "ncpamixer" , group = "terminal apps" }),
    awful.key({ winkey, altkey }, "b", function () awful.util.spawn( "surf www.youtube.com/c/DistroTube" ) end,
        {description = "surf web browser" , group = "gui apps" }),
    awful.key({ winkey, altkey  }, "c", function () awful.util.spawn( terminal.." -e cmus" ) end,
        {description = "cmus" , group = "terminal apps" }),
    awful.key({ winkey, altkey }, "e", function () awful.util.spawn( terminal.." -e neomutt" ) end,
        {description = "neomutt email" , group = "terminal apps" }),
    awful.key({ winkey, altkey  }, "f", function () awful.util.spawn( terminal.." -e sh ./.config/vifm/scripts/vifmrun" ) end,
        {description = "vifm" , group = "terminal apps" }),
    awful.key({ winkey, altkey  }, "i", function () awful.util.spawn( terminal.." -e irssi" ) end,
        {description = "irssi" , group = "terminal apps" }),
    awful.key({ winkey, altkey  }, "j", function () awful.util.spawn( terminal.." -e joplin" ) end,
        {description = "joplin" , group = "terminal apps" }),
    awful.key({ winkey, altkey }, "l", function () awful.util.spawn( terminal.." -e lynx --cfg=~/.lynx/lynx.cfg --lss=~/.lynx/lynx.lss -vikeys gopher://distro.tube" ) end,
        {description = "lynx cli browser" , group = "terminal apps" }),
    awful.key({ winkey, altkey }, "m", function () awful.util.spawn( terminal.." -e toot curses" ) end,
        {description = "toot curses" , group = "terminal apps" }),
    awful.key({ winkey, altkey }, "n", function () awful.util.spawn( terminal.." -e newsboat" ) end,
        {description = "newsboat" , group = "terminal apps" }),
    awful.key({ winkey, altkey  }, "p", function () awful.util.spawn( terminal.." -e pianobar" ) end,
        {description = "pianobar" , group = "terminal apps" }),
    awful.key({ winkey, altkey  }, "w", function () awful.util.spawn( terminal.." -e wopr report.xml" ) end,
        {description = "wopr report.xml" , group = "terminal apps" }),
    awful.key({ winkey, altkey  }, "y", function () awful.util.spawn( terminal.." -e youtube-viewer" ) end,
    ...
    --awful.key({ altkey, "Shift"   }, "Tab",  awful.tag.viewprev,
     ...
     -- Tag browsing winkey + tab
    --awful.key({ winkey,           }, "Tab",   awful.tag.viewnext,
        --{description = "view next", group = "tag"}),
    --awful.key({ winkey, "Shift"   }, "Tab",  awful.tag.viewprev,
        --{description = "view previous", group = "tag"}),
   ...
   -- awful.key({ winkey }, "Right", function () lain.util.tag_view_nonempty(1) end,
    ...
    awful.key({ winkey }, "l",
        ...
        --awful.key({ ctrlkey, winkey }, "Down",
            --function()
                --awful.client.focus.global_bydirection("down")
                --if client.focus then client.focus:raise() end
            --end,
            --{description = "focus down", group = "client"}),
        --awful.key({ ctrlkey, winkey }, "Up",
            --function()
                --awful.client.focus.global_bydirection("up")
                --if client.focus then client.focus:raise() end
            --end,
            --{description = "focus up", group = "client"}),
        --awful.key({ ctrlkey, winkey }, "Left",
            --function()
                --awful.client.focus.global_bydirection("left")
                --if client.focus then client.focus:raise() end
            --end,
            --{description = "focus left", group = "client"}),
        --awful.key({ ctrlkey, winkey }, "Right",
            --function()
                --awful.client.focus.global_bydirection("right")
                --if client.focus then client.focus:raise() end
            --end,
    ...
    awful.key({ winkey }, ".", function () awful.screen.focus_relative( 1) end,
    ...
    awful.key({ winkey, "Control" }, "r", function () lain.util.rename_tag() end,
    ...
    -- Standard program
    --awful.key({ winkey, "Alt" }, "r",  function () os.execute("echo 'awesome.restart()' | awesome-client") end,
              --{description = "reload awesome", group = "awesome"}),
    awful.key({ winkey, "Shift"   }, "q",  function () awful.spawn.with_shell( '~/.dmenu/prompt "are you sure?" "killall awesome"' ) end,
              {description = "quit awesome", group = "awesome"}),

    awful.key({ winkey, "Shift"   }, "l",     function () awful.tag.incmwfact( 0.05)          end,
              {description = "increase master width factor", group = "layout"}),
    awful.key({ winkey, "Shift"   }, "h",     function () awful.tag.incmwfact(-0.05)          end,
              {description = "decrease master width factor", group = "layout"}),
    awful.key({ winkey,    }, "Left",     function () awful.tag.incnmaster( 1, nil, true) end,
              {description = "increase the number of master clients", group = "layout"}),
    awful.key({ winkey,   }, "Right",     function () awful.tag.incnmaster(-1, nil, true) end,
    ...
    awful.key({ winkey,           }, "v", function () awful.layout.inc( 1)                end,
    ...
    --awful.key({ winkey, }, "z", function () awful.screen.focused().quake:toggle() end,
    ...
    --awful.key({ winkey }, "c", function () awful.spawn.with_shell("xsel | xsel -i -b") end,
              --{description = "copy terminal to gtk", group = "hotkeys"}),
    ---- Copy clipboard to primary (gtk to terminals)
    --awful.key({ winkey }, "v", function () awful.spawn.with_shell("xsel -b | xsel") end,
    ...
    awful.key({ altkey, "Shift" }, "z",
              function ()
				  --for s in screen do
					  --awful.screen.focused().swap(awful.screen.focused(), s)
				  --end
				  awful.screen:swap(awful.screen.focused(), awful.screen.focused().get_next_in_direction("right"))
              end,
...
clientkeys = my_table.join(
    ...
    awful.key({ winkey, "Shift"   }, "f",
        function (c)
            c.maximized = not c.maximized
            c:raise()
        end,
        {description = "toggle fullscreen", group = "client"}),
    awful.key({ winkey,           }, "w",      function (c) c:kill()                         end,
              {description = "close", group = "client"}),
    awful.key({ winkey, "Shift" }, "space",  awful.client.floating.toggle                     ,
...
client.connect_signal("manage", function (c)
	...
	elseif c.class == "Vivaldi-snapshot" then
		awful.titlebar.hide(c)
...
client.connect_signal("property::maximized", function(c)
	...
	--else
		--awful.titlebar.hide(c)
	--end
	--if c.class == "Vivaldi-snapshot" then
		--awful.titlebar.hide(c)
		--awful.titlebar.hide(c, "bottom")
	--end
	border_adjust(c)
