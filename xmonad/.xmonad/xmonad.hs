...
myBrowser :: String
myBrowser = "vivaldi-snapshot "  -- Sets qutebrowser as browser
...
myStartupHook = do
    ...
    --spawnOnce "trayer --edge bottom --align right --widthtype request --padding 8 --SetDockType true --SetPartialStrut true --expand true --monitor 0 --transparent true --alpha 0 --tint 0x282c34  --height 28 &"
    spawnOnce "stalonetray &"
    spawnOnce "xkbcomp ~/input.xkb $DISPLAY"
    spawnOnce "xset r rate 300 25"
    spawnOnce "xsetroot -cursor_name left_ptr"
    spawnOnce "( pgrep -x alttab > /dev/null ) || (sleep 2 && alttab -w 1 -d 1 -mk Alt_L &)"
...
myKeys =
    ...
    -- KB_GROUP Increase/decrease spacing (gaps)
        , ("C-S-j", decWindowSpacing 4)         -- Decrease window spacing
        , ("C-S-k", incWindowSpacing 4)         -- Increase window spacing
        , ("C-S-h", decScreenSpacing 4)         -- Decrease screen spacing
        , ("C-S-l", incScreenSpacing 4)         -- Increase screen spacing
    ...
    -- KB_GROUP Layouts
        , ("M-v", sendMessage NextLayout)           -- Switch to next layout
        , ("M-S-f", sendMessage (MT.Toggle NBFULL) >> sendMessage ToggleStruts) -- Toggles noborder/full
        --, ("M-<Space>", spawn "xkb-switch -n") -- Toggles noborder/full
...
main = do
    ...
    xmonad $ ewmh def
		...
		`removeKeysP` ["M-<Space>"]
		`removeKeysP` ["M-S-<Space>"]
		`removeKeysP` ["M-q"]
		`removeKeysP` ["M-p"]
		`removeKeysP` ["M-n"] -- resize viewed windows to the correct size
		`removeKeysP` ["M-<Tab>"] -- Move focus to the next window
		`removeKeysP` ["M-S-<Tab>"] -- Move focus to the previous window
		`removeKeysP` ["M-m"] -- Move focus to the master window
